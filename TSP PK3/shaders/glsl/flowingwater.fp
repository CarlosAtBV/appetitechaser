...
vec4 Process(vec4 color)
{
	...
	vec4 colorCombined = mix(colorDarkWaves, colorRipples, colorDarkBits.r);
	
	float rampX = clamp(colorCombined.r, 0.01, 0.99);
	vec4 colorRamp = texelFetch(texRamp, ivec2(int(rampX * textureSize(texRamp, 0).x), 0), 0);
	vec4 finalColor = mix(texture(texUnder, rippleUVUnder), colorRamp, min(0.5 + colorCombined.r, 1.0));
